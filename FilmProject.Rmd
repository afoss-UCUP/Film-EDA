...
#make a variable to check path dependence of total gross

gross_tier <- cut(movie_df[relative_week == 1, gross],
                  breaks = quantile(movie_df[, gross],
                                    c(0, .05, .33, .66, .95, 1)))

gross_tier <- data.table('title' = movie_df[relative_week == 1, title], 
                        gross_tier)

gross_tier[,gross_tier:= factor(gross_tier, ordered = T)]

movie_df <- merge(movie_df, gross_tier, by = 'title')



open_tier <- cut(movie_df[relative_week == 1, weekly_gross],
                  breaks = quantile(movie_df[, weekly_gross],
                                    c(0, .05, .33, .66, .95, 1)))

open_tier <- data.table('title' = movie_df[relative_week == 1, title], 
                        open_tier)

open_tier[,open_tier:= factor(open_tier, ordered = T)]
...
critic_tier <- cut(movie_df[relative_week == 1, critics_avg],
                  breaks = quantile(movie_df[, critics_avg],
                                    c(0, .05, .33, .66, .95, 1), na.rm = ))

critic_tier <- data.table('title' = movie_df[relative_week == 1, title], 
                        critic_tier)

critic_tier[,critic_tier:= factor(critic_tier, ordered = T)]

movie_df <- merge(movie_df, critic_tier, by = 'title')



MV_gross_range <- get_chart_range(movie_df, 'weekly_gross', .01, .999)
p44 <- ggplot(aes(x = relative_week, y = weekly_gross, color = critic_tier),
              data = movie_df[!is.na(weekly_gross), ]) +
  geom_point(alpha = .25, position = position_jitter(h = 0)) +
  coord_cartesian(xlim = c(.5, 8.5), ylim = MV_gross_range) +
  geom_line(stat = 'summary', fun.y = 'median', size = 1) +
  geom_line(stat = 'summary', fun.y = quantile, prob = .25, linetype = 2, size = .5) +
  geom_line(stat = 'summary', fun.y = quantile, prob = .75, linetype = 2, size = .5) +
  scale_y_log10() +
  ylab('log10(weekly_gross)')

MV_theater_range <- get_chart_range(movie_df, 'weekly_theaters', .01, .999)
p45 <- ggplot(aes(x = relative_week, y = weekly_theaters, color = critic_tier),
              data = movie_df[!is.na(weekly_theaters), ]) +
  geom_point(alpha = .1, position = position_jitter(h = 0)) +
  coord_cartesian(xlim = c(.5, 8.5), ylim = MV_theater_range) +
  geom_line(stat = 'summary', fun.y = 'median', size = 1) +
  geom_line(stat = 'summary', fun.y = quantile, prob = .25, linetype = 2, size = .5) +
  geom_line(stat = 'summary', fun.y = quantile, prob = .75, linetype = 2, size = .5) +
  scale_y_log10() +
...
MV_eff_range <- get_chart_range(movie_df, 'daily_theater_eff', .01, .999)
p46 <- ggplot(aes(x = relative_week, y = daily_theater_eff, color = critic_tier),
              data = movie_df[!is.na(daily_theater_eff), ]) +
  geom_point(alpha = .1, position = position_jitter(h = 0)) +
  coord_cartesian(xlim = c(.5, 8.5), ylim = MV_eff_range) +
  geom_line(stat = 'summary', fun.y = 'median', size = 1) +
  geom_line(stat = 'summary', fun.y = quantile, prob = .25, linetype = 2, size = .5) +
  geom_line(stat = 'summary', fun.y = quantile, prob = .75, linetype = 2, size = .5) +
  scale_y_log10() +
...
movie_df[between(release_date,as.Date(paste(year,5,16,sep = '-')),
                                  as.Date(paste(year,8,15,sep = '-'))),
         season := 'Summer']
movie_df[between(release_date, as.Date(paste(year,8,16,sep = '-')),
                                  as.Date(paste(year,11,15,sep = '-'))),
         season := 'Fall']
movie_df[between(release_date, as.Date(paste(year,2,16,sep = '-')),
                                  as.Date(paste(year,5,15,sep = '-'))),
         season := 'Spring']
movie_df[between(release_date, as.Date(paste(year,11,16,sep = '-')),
                                  as.Date(paste(year,12,31,sep = '-')))
         | between(release_date, as.Date(paste(year,1,1,sep = '-')),
...
Given the importance of novelty (the relative week of release), it makes sense to continue examining our key metrics along those lines.  We can 
