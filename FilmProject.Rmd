...
## Numeric Measures: Film-Week Variables
...
## Numeric Measures: Unique Film Variables
...
#weekly gross
p44 <- build_tiered_week_plot(movie_df, 'weekly_gross', 'gross_tier')
#weekly theaters
p45 <- build_tiered_week_plot(movie_df, 'weekly_theaters', 'gross_tier')
#daily theater eff
p46 <- build_tiered_week_plot(movie_df, 'daily_theater_eff', 'gross_tier')
...
grid.arrange(arrangeGrob(p44, p45, p46, nrow = 1),
             ...
             top = 'Key Metrics and Perfect Foresigt: Release Lifecycle')
...
#weekly gross
p47 <- build_tiered_week_plot(movie_df, 'weekly_gross', 'open_tier')
#weekly theaters
p48 <- build_tiered_week_plot(movie_df, 'weekly_theaters', 'open_tier')
#daily theater eff
p49 <- build_tiered_week_plot(movie_df, 'daily_theater_eff', 'open_tier')
...
grid.arrange(arrangeGrob(p47, p48, p49, nrow = 1),
             joint_legend, nrow = 2, heights = c(10, 1),
             top = 'Opening Week: Release Lifecycle')
...
#weekly gross
p50 <- build_tiered_week_plot(movie_df, 'weekly_gross', 'critic_tier')
#weekly theaters
p51 <- build_tiered_week_plot(movie_df, 'weekly_theaters', 'critic_tier')
#daily theater eff
p52 <- build_tiered_week_plot(movie_df, 'daily_theater_eff', 'critic_tier')
...
grid.arrange(arrangeGrob(p50, p51, p52, nrow = 1),
             joint_legend, nrow = 2, heights = c(10, 1),
             top = 'Critical Acclaim: Release Lifecycle')
...
```{r echo=FALSE, Critical_Sleepers, fig.width = 12, fig.height = 3, htmlcap = "Fig. 15: Cross-section of critic tiers by opening tier."}

ggplot(aes(x = critic_tier, fill = open_tier),
...
#weekly gross
p53 <- build_tiered_week_plot(movie_df, 'weekly_gross', 'cast_tier')
#weekly theaters
p54 <- build_tiered_week_plot(movie_df, 'weekly_theaters', 'cast_tier')
#daily theater eff
p55 <- build_tiered_week_plot(movie_df, 'daily_theater_eff', 'cast_tier')

# grab joint legend
joint_legend <- grab_legend(p53)

# remove individual legends 
p53 <- p53 + theme(legend.position="none")
p54 <- p54 + theme(legend.position="none")
p55 <- p55 + theme(legend.position="none")


grid.arrange(arrangeGrob(p53, p54, p55, nrow = 1),
             joint_legend, nrow = 2, heights = c(10, 1),
             top = 'Casting Call: Release Lifecycle')
...
* Cast derived measures have the most monotonic relationship with our perfect foresight measures
* causality is hard to tease out, but given the fact that we can tell a simple compelling story and already know our talent measure has lookahead bias, I'd be surprised if we did not see a stable, monotonic relationship

You may be wondering how different our cast and critic quality metrics are from one another.  It is easy to imagine that there is one common factor driving cast, critic score and gross (some single 'awesomeness' factor). Here is a simple chart that provides an answer.

```{r echo=FALSE, Multidimensional_Proof, fig.width = 12, fig.height = 3, htmlcap = "Fig. 16: The cross-sections of cast tiers, critic score tiers and log10 total gross."}

ggplot(aes(x = cast_tier, y = gross, color = critic_tier), 
       data = na.omit(unique(movie_df[relative_week == 1,
                                      list(critic_tier, cast_tier, gross)]))) + 
  geom_boxplot(position = 'dodge') +
  scale_y_log10() +
  ggtitle('Looking for "Awesomeness"') +
  ylab('log10(gross)')
  
```

Looking at the above, I notice that the relationship with gross is essentially monotonic across casting tier and across critic tier.  If cast tier and critic tier captured the same exact information about gross, we would not observe monotonicity with gross along both dimensions. Clearly, the measures examined here are different (though this does not preclude a better representation using some latent factor...work for another time).

### Seasonality and Other Films

```{r echo=FALSE, Multidimensional_Proof, fig.width = 12, fig.height = 3, htmlcap = "Fig. 17: The cross-sections of cast tiers, critic score tiers and total gross."}
...
movie_df[,cum_comp_theaters:= mean(comp_week_critics, na.rm = T), by = title]

ggplot(aes(x = season, y = weekly_gross, color = comp_tier), 
       data = na.omit(
         unique(movie_df[relative_week == 1,list(season, comp_tier, weekly_gross)]))
       ) + 
  geom_boxplot(position = 'dodge') +
  #coord_cartesian(ylim = gross_range) +
  scale_y_log10()
...
ggplot(aes(x = critic_tier, y = talent_cum_pics, color = gross_tier), data = unique(movie_df[!is.na(critic_tier) & !is.na(open_tier), list(talent_cum_pics,open_tier,critic_tier,gross_tier)])) +
  geom_boxplot(position = 'dodge')
