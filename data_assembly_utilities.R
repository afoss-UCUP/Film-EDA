...
get_talent_from_td_tags <- function(td_tags, json_box){
  ...
  json_box$talent$actors <- 
    gsub('\\*', '',
         unlist(strsplit(td_tags[which(td_tags == 'Actors:') + 1], '\n')))
...
grab_mojo_movies_data <- function(filmid){
  ...
  json_box$release_date <- as.character(as.Date(json_box$release_date,
                                                '%B %d, %Y'))
...
build_historic <- function(dl_file){
  ...
  weekend_links <- read_html(dl_file) %>%
    html_nodes(weekend_links_column) %>% 
    html_attrs()
  ...
  weekend_days <- read_html(dl_file) %>% 
    html_nodes(weekend_days_column) %>%
    html_text()
  ...
  weekend_grosses <- read_html(dl_file) %>%
    html_nodes(weekend_gross_column) %>%
    html_text()
  weekend_grosses <- na.omit(as.numeric(gsub('[[:punct:]]|[[:alpha:]]', 
                                             '', 
                                             weekend_grosses[-1])))
  
  weekend_theater_column <- 'tr+ tr td:nth-child(5) font'
  weekend_theaters <- read_html(dl_file) %>% 
    html_nodes(weekend_theater_column) %>%
    html_text()
  weekend_theaters <- na.omit(as.numeric(gsub('[[:punct:]]', 
                                              '', 
                                              weekend_theaters)))
  
  weekend_release_column <- 'tr+ tr td:nth-child(9) font'
  weekend_release <- read_html(dl_file) %>% 
    html_nodes(weekend_release_column) %>%
    html_text()
  weekend_release <- na.omit(as.numeric(gsub('[[:punct:]]',
                                             '', 
                                             weekend_release)))
...
compose_weekly_dataframe <- function(weekly_data){
  ...
  for(i in c(1:length(weekend_release))){
    out[[i]] <- build_week(i, weekend_release, 
                           weekend_links,
                           weekend_days, 
                           weekend_grosses,
                           weekend_theaters)
...
build_week <- function(i, rel, lnks, dys, gro, thea){
  ...
  if(!is.na(check)){
    ...
    begin <- paste(dates[1], ', ', year, sep = '')
    if(substr(dates[1],1,3) %like% 'Dec' 
       & substr(dates[2], 1, 3) %like% 'Jan'){
...
# function that generates a likely Metacritic id from a title
...
grab_metacritic_data <- function(filmid){
  ...
  dl_url <- getURLContent(paste(baseurl, filmid,sep=''), 
                          useragent = 'Moviefan-via-R')
  closeAllConnections()
  main_vals <- read_html(dl_url) %>% 
    html_nodes('a div') %>% 
    html_text()
  dl_url <- getURLContent(paste(baseurl, filmid ,'/critic-reviews', sep=''),
                          useragent = 'Moviefan-via-R')
  closeAllConnections()
  critic_scores <- read_html(dl_url) %>% 
